...
test_data = pd.read_csv('Data/log_ret.csv', parse_dates=['date'])
...
def summarize_anomalies(anomaly_dir, print_info = False, plot = False, instreamlit = False):
    '''
...
def plot_anomalies(anomaly_dir, model_name, log_returns = test_data, instreamlit= False):
    ...
    if instreamlit:
        ...
        plt.show()  # Show the plot in a standard Matplotlib window


# Plot the distribution of anomalies per models 
def plot_anomaly_distribution(anomaly_dir, instreamlit=False):
    models = __get_models(anomaly_dir)
...
def get_jaccard(anomaly_dir, instreamlit=False):
    models = __get_models(anomaly_dir)
    ...
    sns.heatmap(jaccard_df.astype(float), annot=True, cmap='Blues', fmt=".2f", ax=ax)
    ax.set_title('Jaccard Similarity Index Between Models')
...
def majority_anomalies_consensus(anomaly_dir, consensus_threshold = 4, plot = False, log_return = test_data):
    ...
    if plot:
        ...
        plot_anomalies(anomaly_cons_dict, 'Majority Consensus', instreamlit=True)
