...
if __name__ == "__main__":
    ...
    # roll_volume = volume.rolling(20).mean().dropna()
    ...
    roll_volume = valid_volume.rolling(20).mean().dropna()
    ...
    for i, setcor in enumerate(mae_df.columns):
        print(f"{setcor}: {np.corrcoef(roll_volume.iloc[:, i], mae_df.iloc[:, i])[0, 1]}")

    fig, axes = plt.subplots(3, 4, figsize=(15, 10), sharex=True)
    axes = axes.flatten()

    for i, column_name in enumerate(mae_df.columns):
        axes[i].scatter(np.log(roll_volume).iloc[:, i], mae_df.iloc[:, i], color="red", alpha=0.6)
        axes[i].set_title(f"Sector: {column_name}")
        axes[i].set_xlabel("Volume")
        axes[i].set_ylabel("MAE")

    # Remove extra subplots
    for j in range(len(mae_df.columns), len(axes)):
        fig.delaxes(axes[j])

    plt.suptitle('Volume vs MAE Across Sectors', fontsize=16)
